...
راه‌اندازی (یک‌بار)
یک ریپازیتوری جدید در گیت‌هاب بساز.
همه‌ی فایل‌های این پوشه را در آن آپلود کن (Add file → Upload files، یا git push).
برو به Settings → Pages.
زیر «Build and deployment»، Source را روی Deploy from a branch بگذار و شاخه‌ی `main` / پوشه‌ی `/ (root)` را انتخاب کن.
چند دقیقه صبر کن؛ آدرس سایت: `https://USERNAME.github.io/REPO-NAME/`
